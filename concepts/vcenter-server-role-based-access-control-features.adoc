...
Você pode visualizar as ferramentas do ONTAP para as funções padrão do VMware vSphere selecionando *Roles* na página inicial do vSphere Client. As funções que as ferramentas do ONTAP para VMware vSphere fornecem permitem que você execute as seguintes tarefas:
