...
sidebar: sidebar 
keywords: add storage backend, ONTAP tools storage backend, local and global storage backend, MetroCluster storage backend 
summary: Adicione e gerencie backends de armazenamento localmente para uma instância vCenter ou globalmente para descoberta em todo o cluster nas ferramentas ONTAP. 
---
= Adicionar um backend de armazenamento às ferramentas ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Consulte o artigo da base de conhecimento: https://kb.netapp.com/data-mgmt/OTV/VSC_Kbs/OTV_10_What_is_storage_backend_global_and_local_scope["OTV 10: O que é o escopo global e local do backend de armazenamento"^] para obter mais informações sobre backends de armazenamento globais e locais.

Se o suporte ao NetApp MetroCluster estiver habilitado no ONTAP, integre ambos os clusters de origem e destino como backends de storage locais ou globais.
...
https://kb.netapp.com/data-mgmt/OTV/VSC_Kbs/OTV_10_What_is_storage_backend_global_and_local_scope["OTV 10: O que é o escopo global e local do backend de armazenamento"^]
