...
Ao registrar uma instância de vCenter Server com ONTAP tools Manager, certifique-se de que o certificado do vCenter Server inclua uma extensão válida de Subject Alternative Name (SAN). A extensão SAN deve conter entradas de DNS e de endereço IP. Por exemplo:
...
----
Se o certificado do servidor vCenter não incluir uma extensão SAN, ou se a extensão SAN não contiver os valores corretos de DNS ou endereço IP, as operações das ONTAP tools poderão falhar devido a erros de validação do certificado.
