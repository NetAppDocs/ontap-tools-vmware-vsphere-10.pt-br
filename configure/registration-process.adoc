...
summary: Registre o provedor VASA em uma instância do servidor vCenter usando a interface remota do plug-in ONTAP tools. 
---
= Registre o provedor VASA com uma instância do vCenter Server nas ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
As configurações do provedor VASA mostram o status de registro para o vCenter Server selecionado.
...
. Insira um nome e as credenciais do VASA Provider. O nome de usuário pode conter apenas letras, números e sublinhados. A senha deve ter entre 8 e 256 caracteres.
