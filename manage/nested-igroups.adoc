...
[role="lead"]
Se você gerencia VMs de ferramentas ONTAP e sistemas de armazenamento ONTAP , é importante entender como os igroups se comportam, especialmente ao mover armazenamentos de dados de ambientes não gerenciados por ferramentas ONTAP para aqueles que são. Esta página explica como os grupos de interação (igroups) são atualizados durante esse processo.

A partir do ONTAP tools for VMware vSphere 10.4, o ONTAP tools cria e mantém automaticamente objetos ONTAP e vCenter, simplificando o gerenciamento de datastores em ambientes de datacenter VMware.
...
NOTE: Somente igroups personalizados podem ser reutilizados. Igroups criados durante a migração do ONTAP tools 9.x (igroups migrados) não podem ser reutilizados e, a partir do ONTAP tools 10.5P2, são excluídos automaticamente quando não estão mais em uso.
