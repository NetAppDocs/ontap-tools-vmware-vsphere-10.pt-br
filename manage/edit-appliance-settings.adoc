...
* Certifique-se de que o hot add e o hot plug de memória da CPU estejam ativados durante a implantação. Você pode ativá-los no menu do vCenter client se eles não tiverem sido ativados durante a implantação. Siga estas instruções para habilitá-los a partir do cliente vCenter:
+
** Navegue até as ferramentas do ONTAP para a máquina virtual VMware vSphere em *inventários* e selecione *Editar configurações*.
