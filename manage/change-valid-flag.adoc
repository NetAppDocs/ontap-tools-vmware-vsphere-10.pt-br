---
permalink: manage/change-valid-flag.html 
sidebar: sidebar 
keywords: certificate validation flag, ONTAP tools certificate check, maintenance console certificate settings, TLS validation toggle 
summary: Altere o sinalizador de validação do certificado usando o console de manutenção. 
---
= Alterar o indicador de validação do certificado nas ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Por padrão, o sinalizador de validação de certificado está habilitado (definido como verdadeiro).  Você pode definir o sinalizador de validação do certificado de backend de armazenamento ONTAP como falso se precisar ignorar as verificações do certificado SAN.  Esta configuração não é aplicável aos certificados do vCenter Server.

.Antes de começar
Você precisa ter credenciais de login de usuário de manutenção.

.Passos
. Faça login no servidor vCenter.
. Localize a máquina virtual do ONTAP tools e inicie o console da web.
. Faça login usando as `maint` credenciais.
. Digitar `1` Para selecionar o menu *Configuração do Aplicativo*.
. Digitar `3` para alterar o sinalizador de validação do certificado.
+
O console de manutenção mostra o status do sinalizador de validação do certificado e solicita que você o altere.

. Digite 'y' para alternar o sinalizador ou 'n' para cancelar.


Ao ativar o sinalizador de validação de certificado (definido como verdadeiro), as ferramentas ONTAP verificam se todos os backends de armazenamento usam certificados com um Nome Alternativo do Assunto (SAN).  Se algum backend usar um certificado sem SAN, você não poderá ativar a validação de certificado.  Antes de ativar essa opção, verifique se todos os servidores de armazenamento utilizam certificados baseados em SAN.  Se você desativar o sinalizador de validação de certificado (definido como falso), as ferramentas ONTAP ignorarão a validação de certificado para todos os backends de armazenamento configurados.



[[verify-san-based-certificates-for-storage-backends]]
== Verifique certificados baseados em SAN para backends de armazenamento

Para garantir uma comunicação segura e a validação adequada, verifique se todos os storage backends utilizam certificados baseados em SAN:

. Verifique se o certificado de gerenciamento do ONTAP inclui uma entrada de Nome Alternativo do Assunto (SAN).
. Confirme se as entradas SAN correspondem ao endereço IP de gerenciamento do ONTAP ou ao nome DNS, ou a ambos.
. Certifique-se de que os detalhes usados para integrar o ONTAP correspondam ao endereço IP ou nome DNS na entrada SAN do certificado.


Seguir esses passos ajuda a evitar problemas de validação de certificados e garante que o sistema ONTAP esteja integrado de forma segura.

O seguinte certificado de exemplo mostra as informações decodificadas:

image:../media/sample-cert.png["Exemplo de certificado"]
