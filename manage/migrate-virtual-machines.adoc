...
São suportados snapshots de máquinas virtuais sem memória. Se uma máquina virtual tiver um snapshot com memória, a máquina virtual não poderá ser protegida.

Não é possível proteger máquinas virtuais desprotegidas que contenham snapshots de memória. Exclua os snapshots de memória antes de ativar a proteção para a máquina virtual.

Para uma máquina virtual Windows com armazenamento ASA r2, os snapshots da máquina virtual são somente leitura. Ao ligar a máquina virtual, VASA Provider cria um LUN a partir do snapshot somente leitura e habilita IOPS. Ao desligar a máquina virtual, VASA Provider exclui o LUN e desabilita IOPS.
