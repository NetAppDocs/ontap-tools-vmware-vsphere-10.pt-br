...
keywords: REST API fundamentals, HTTP methods, JSON, authentication 
...
---
= Saiba mais sobre a API REST do ONTAP tools
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
ONTAP tools for VMware vSphere 10 expõe um único endereço IP externo (anteriormente load balancer) que fornece um gateway para os recursos da máquina virtual. Você fornece esse endereço durante a configuração inicial. O endereço é usado pela interface do ONTAP tools Manager e para acessar diretamente a página de documentação do Swagger e a API REST.
