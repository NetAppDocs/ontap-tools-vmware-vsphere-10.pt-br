...
summary: Preencha a lista de verificação pré-implantação para ONTAP tools, incluindo vSphere compatibilidade, dimensionamento, certificados e requisitos de rede. 
---
= Requisitos de pré-implantação para ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| vSphere versão, versão do ONTAP e versão do host ESXi são compatíveis com a versão das ONTAP tools. | ❏ Sim ❏ não 
...
| Endereço IP do ONTAP tools (anteriormente load balancer). Este endereço IP externo é usado para acessar a interface do ONTAP tools Manager em  `\https://<ip>:8443/virtualization/ui/`, enviar chamadas de API e acessar o plug-in do ONTAP tools a partir da interface do vCenter. |  


| Endereço IP virtual do ONTAP tools (anteriormente interconexão de nó). Este endereço IP é atribuído ao servidor da API do Kubernetes e atua como o endereço IP virtual para o plano de controle do Kubernetes em implantações de nó único e de vários nós. Ele é usado para comunicação interna do cluster e gerenciamento do Kubernetes, e deve ser diferente do endereço IP do ONTAP tools. |  
...
| Endereço IPv4 para o nó de gerenciamento das ONTAP tools. Em uma implantação de nó único, esse endereço é mapeado para o nome de host especificado para a máquina virtual das ONTAP tools. Em uma implantação HA, você deve fornecer endereços IPv4 adicionais para as duas máquinas virtuais adicionais das ONTAP tools. |  
