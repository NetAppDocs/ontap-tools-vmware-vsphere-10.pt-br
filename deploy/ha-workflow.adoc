...
sidebar: sidebar 
keywords: high availability deployment, HA workflow, VASA Provider, VMware Live Site Recovery 
summary: 'Expanda uma implementação do ONTAP tools para uma configuração de alta disponibilidade (HA) para aumentar a resiliência e suportar mais contêineres por serviço.' 
---
= Fluxo de trabalho de implantação de alta disponibilidade para ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="quick-margin-para"]
Para configurar os datastores vVols, você deve habilitar o serviço VASA Provider. Registre o provedor VASA no vCenter e certifique-se de que suas políticas de armazenamento atendam aos requisitos de HA, incluindo configurações adequadas de rede e armazenamento.
