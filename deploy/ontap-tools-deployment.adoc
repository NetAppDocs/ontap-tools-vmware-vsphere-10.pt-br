...
|===
. No campo *Selecione um nome e pasta*, insira o nome da máquina virtual e escolha sua localização. Para habilitar a capacidade de usar o recurso HA, selecione a opção *Personalizar o hardware desta máquina virtual*, que ativará uma etapa adicional chamada *Personalizar hardware* antes de prosseguir para a janela *Pronto para concluir*.
...
. Na janela *Customize hardware*, ative as opções *CPU hot add* e *Memory hot plug* para permitir a funcionalidade HA.
